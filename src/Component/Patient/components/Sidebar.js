  ...
  return (
    <nav className={`${isOpen ? 'block' : 'hidden'} lg:block lg:w-52 bg-white h-screen fixed top-0 left-0 z-50`}>
      ...
      <ul className="text-[#7F8F98]">
        ...
        <li className={`pl-6 p-3 lg:pb-3 hover:text-[#3497F9] flex ${activeLink === '/patient/messages' ? 'bg-blue-100 text-[#3497F9]' : ''}`}><RiMessage2Fill className="w-5 h-5 mr-3" /><span className="cursor-pointer text-sm" onClick={() => handleItemClick('/patient/messages')}><Link to="">Messages</Link></span></li>
